...
#pragma region CollisionFunctionality
	...
	{
		return GetDistance(Point2f{ point1X, point1Y }, Point2f{ point2X, point2Y });
	}
	bool IsPointInCircle(const Point2f& pos, const Ellipsef& circle)
	{
		return GetDistance(pos, circle.center) <= circle.radiusX;
		
	}
	bool IsPointInCircle(const float posX, const float posY, const Point2f& circlePos, const float radius)
	{
		return IsPointInCircle(Point2f{ posX, posY }, Ellipsef{ circlePos, radius, radius });
	}
	bool IsPointInRect(const Point2f& pos, const Rectf rect)
	{
		return(pos.x >= rect.left && pos.y >= rect.top && pos.x <= rect.left + rect.width && pos.y <= rect.top + rect.height);
	}
	bool IsPointInRect(const float& posX, const float& posY, const float& left, const float& top, const float& width, const float& height)
	{
		return IsPointInRect(Point2f(posX, posY), Rectf(left, top, width, height));
	}
	...
	bool IsOverLapping(const Ellipsef& circle1, const Ellipsef& circle2)
	{
		return (GetDistance(circle1.center, circle2.center) > circle1.radiusX + circle2.radiusX);
